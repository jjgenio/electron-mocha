...
import { app, ipcMain as ipc, powerSaveBlocker } from 'electron';
import { createWindow } from 'electron-window';
import { errors, helpers, ONE_AND_DONES } from './mocha';
import run from 'mocha/lib/cli/run';
import ansi from 'ansi-colors';
import util from 'util';

export const command = run.command
export const describe = 'Run tests with Electron-Mocha'
  ...
  try {
    ...
    if (!argv.renderer) {
      ...
      const customWindowOptions = argv['window-config']
        ? await import(resolve(argv['window-config']))
        : {}
      ...
      const win = createWindow({
        ...
        webPreferences: {
          ...
          preload: join(url.fileURLToPath(new URL('.', import.meta.url)), '..', 'renderer', 'run.js')
        }
      })
      ...
      win._loadURLWithArgs(
        argv.url ? argv.url : join(url.fileURLToPath(new URL('.', import.meta.url)), '../renderer/index.html'),
        argv,
        () => {
          if (argv.inspect) {
            win.show()
